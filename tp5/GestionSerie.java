...
class GestionSerie {
    ...
    public GestionSerie(Serie serie, Episode episode, Personne personne, RoleEpisode roleEpisode) throws Tp5Exception{
    ...
    public void ajoutSerie(String titre, Date dateSortie, String realisateur) throws Exception {
        try {
            ...
            if (serie.existe(titre, dateSortie)) {
                throw new Tp5Exception("Impossible d'ajouter, la serie " + titre + " paru le " + dateSortie + " existe deja.");
            }
            ...
            if (realisateurNaissance.after(dateSortie)){
                throw new Tp5Exception("Impossible d'ajouter, le réalisateur est né le " + realisateurNaissance + 
    ...
    public void ajoutEpisode(String titre, String titreSerie, Date anneeSortieSerie, int noSaison, int noEpisode, String description, Date dateDiffusion) throws Exception {
        try {
            ...
            if (!serie.existe(titreSerie, anneeSortieSerie)) {
                throw new Tp5Exception("Impossible d'ajouter, la serie " + titreSerie + " paru le " + anneeSortieSerie + " n'existe pas.");
            }
            //verifie si l episode existe deja
            if (episode.existe(titreSerie, anneeSortieSerie, noSaison, noEpisode)) {
                throw new Tp5Exception("Impossible d'ajouter, l'episode existe deja: " + titre + " " + dateDiffusion + " saison: " + noSaison + " episode: " + noEpisode);
            } 
            // verifie si le no saison est valide
            if(noSaison > serie.getSerie(titreSerie, anneeSortieSerie).getNbSaison())
            {
                throw new Tp5Exception("Impossible d'ajouter, il n'y a pas de saison " + noSaison + " pour la serie: " + titreSerie);
            }
            //verifie que le no episode est valide
            if(noEpisode < 1){
                throw new Tp5Exception("Impossible d'ajouter, l'episode No" + noEpisode + " ne peut etre ajouter car : le numero doit etre plus grand ou egal a 1.");
            }
            
            // S'assure que l'épisode d'avant existe
            if (noEpisode > 1) {
                if (!episode.existe(titreSerie, anneeSortieSerie, noSaison, noEpisode-1)){
                    throw new Tp5Exception("Impossible d'ajouter, l'episode No" + noEpisode + " ne peu etre ajouter car : l'episode No " + (noEpisode-1) + " est manquant.");
                }
            }
    ...
    public void ajoutRoleAEpisode(String serieTitre, Date serieDate, int noSaison, int noEpisode, 
        ...
        try {
            ...
            if(!serie.existe(serieTitre, serieDate)){
                throw new Tp5Exception("Impossible d'ajouter, la serie : " + serieTitre + " n'existe pas.");
            }
            //verifie que l episode existe
            if(!episode.existe(serieTitre, serieDate, noSaison, noEpisode)){
                throw new Tp5Exception("Impossible d'ajouter, l'episode no : " + noEpisode + " de la saison : " + noSaison + " n'existe pas.");
            }
            //verifie que l acteur existe
            if(!personne.existe(acteur)){
                throw new Tp5Exception("Impossible d'ajouter, l'acteur : " + acteur + " n'existe pas.");
            }
            ...
            if (tupleActeur.getDateNaissance().after(serieDate)){
                throw new Tp5Exception("Impossible d'ajouter, l'acteur " + acteur + " est né le: " + tupleActeur.getDateNaissance() + 
            ...
            if(roleEpisode.existe(serieTitre, serieDate, noSaison, noEpisode, acteur, roleActeur)){
                throw new Tp5Exception("Impossible d'ajouter, l'acteur : " + acteur + " joue deja dans l'episode " + 
            ...
            if(roleEpisode.existeRole(serieTitre, serieDate, noSaison, noEpisode, roleActeur)){
                throw new Tp5Exception("Un autre acteur joue deja le role : " + roleActeur + " dans l'episode " + 
    ...
    public void afficherActeursSerie(String serieTitre, Date serieDate) throws SQLException, Tp5Exception {
        if(!serie.existe(serieTitre, serieDate)){
            throw new Tp5Exception("Impossible d'afficher, la serie " + serieTitre + " paru le " + serieDate + " n'existe pas.");
        }
